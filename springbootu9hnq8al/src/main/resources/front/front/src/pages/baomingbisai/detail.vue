	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/baomingbisai?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('baomingbisai','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('baomingbisai','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('baomingbisai','分组')" 
							@click="onAcross('bisaifenzu','','','','')" type="warning">
							分组
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">报名备注</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.baomingbeizhu"></div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`baomingbisai`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/baomingbisai', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/baomingbisaiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此报名比赛？') .then(_ => {
					this.$http.post('baomingbisai/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
