	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zhuangbeixinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
				...
				<div class="info">
					<div class="title-item">
						<div class="detail-title">
							{{detail.zhuangbeimingcheng}}
						</div>
					</div>
					<div class="item">
						<div class="lable">装备编号</div>
						<div class="text "  >{{detail.zhuangbeibianhao}}</div>
					</div>
					<div class="item">
						<div class="lable">装备分类</div>
						<div class="text "  >{{detail.zhuangbeifenlei}}</div>
					</div>
					<div class="item">
						<div class="lable">装备数量</div>
						<div class="text "  >{{detail.zhuangbeishuliang}}</div>
					</div>
					<div class="item">
						<div class="lable">装备型号</div>
						<div class="text "  >{{detail.zhuangbeixinghao}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zhuangbeixinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zhuangbeixinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zhuangbeixinxi','分配')" 
							@click="onAcross('zhuangbeifenpei','','','[1]','已分配')" type="warning">
							分配
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">装备详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.zhuangbeixiangqing"></div>
			</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.zhuangbeimingcheng;
						if(this.detail.zhuangbeizhaopian) {
							this.detailBanner = this.detail.zhuangbeizhaopian.split(",w").length>1?[this.detail.zhuangbeizhaopian]:this.detail.zhuangbeizhaopian.split(',');
						}
						this.$forceUpdate();
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`zhuangbeixinxi`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zhuangbeixinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zhuangbeixinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此装备信息？') .then(_ => {
					this.$http.post('zhuangbeixinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
