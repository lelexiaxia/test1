	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="分组时间" prop="fenzushijian" >
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.fenzushijian" 
						type="date"
						:readonly="ro.fenzushijian"
						placeholder="分组时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.fenzushijian" label="分组时间" prop="fenzushijian" >
					<el-input v-model="ruleForm.fenzushijian" placeholder="分组时间" readonly></el-input>
				</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.fenzushijian = this.getCurDate()
		},
		methods: {
			// 下载
			download(file){
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='fenzushijian'){
							this.ruleForm.fenzushijian = obj[o];
							this.ro.fenzushijian = true;
							continue;
						}
						if(o=='wanjiazhanghao'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `bisaifenzu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.bisaifenzuCrossAddOrUpdateFlag = false;
											this.parent.search();
