<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="比赛类型" prop="bisaileixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">比赛类型</label>
						<el-select clearable v-model="searchForm.bisaileixing" placeholder="请选择比赛类型" >
							<el-option v-for="(item,index) in bisaileixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('bisaixinxi','比赛类型统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						比赛类型统计
					</el-button>
				</el-row>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('bisaixinxi','报名')" @click="baomingbisaiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已报名')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								报名
							</el-button>
		...
		<baomingbisai-cross-add-or-update v-if="baomingbisaiCrossAddOrUpdateFlag" :parent="this" ref="baomingbisaiCrossaddOrUpdate"></baomingbisai-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="bisaileixingChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import baomingbisaiCrossAddOrUpdate from "../baomingbisai/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				baomingbisaiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			baomingbisaiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.baomingbisaiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.baomingbisaiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.baomingbisaiCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var bisaileixingChart1 = echarts.init(document.getElementById("bisaileixingChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							bisaileixingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								bisaileixingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
