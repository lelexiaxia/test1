	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="胜率" prop="shenglv" >
					<el-select :disabled="ro.shenglv" v-model="ruleForm.shenglv" placeholder="请选择胜率" >
						<el-option
							v-for="(item,index) in shenglvOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="胜率" prop="shenglv" >
					<el-input v-model="ruleForm.shenglv"
						placeholder="胜率" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="MVP" prop="paiming" >
					<el-input v-model.number="ruleForm.paiming" placeholder="MVP" clearable  :readonly="ro.paiming"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="MVP" prop="paiming" >
					<el-input v-model="ruleForm.paiming" placeholder="MVP" readonly></el-input>
				</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shenglv'){
							this.ruleForm.shenglv = obj[o];
							this.ro.shenglv = true;
							continue;
						}
						if(o=='paiming'){
				...
				this.shenglvOptions = "获胜,失败,平局".split(',')
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `bisaichengji/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.bisaichengjiCrossAddOrUpdateFlag = false;
											this.parent.search();
