<template>
	<div class="center-preview">
		...
		<div class="center-info">
			...
			<div class="img-box" v-if="userTableName=='wanjia'">
				<img :src="sessionForm.touxiang?baseUrl + sessionForm.touxiang:require('@/assets/avator.png')">
			</div>
			<div class="info-item1" v-if="userTableName=='wanjia'">
				<span class="icon iconfont "></span>
				<div class="label">玩家账号：</div>
				<div class="text">{{sessionForm.wanjiazhanghao}}</div>
			</div>
			<div class="info-item2" v-if="userTableName=='wanjia'">
				<span class="icon iconfont "></span>
				<div class="label">玩家姓名：</div>
				<div class="text">{{sessionForm.wanjiaxingming}}</div>
			</div>
			<div class="info-item3" v-if="userTableName=='wanjia'">
				<span class="icon iconfont "></span>
				<div class="label">性别：</div>
				<div class="text">{{sessionForm.xingbie}}</div>
			</div>
			<div class="info-item4" v-if="userTableName=='wanjia'">
				<span class="icon iconfont "></span>
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			<el-tab-pane label="个人中心">
				<el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
					<el-form-item class="center-item" v-if="userTableName=='wanjia'" label="玩家账号" prop="wanjiazhanghao">
						<el-input v-model="sessionForm.wanjiazhanghao" placeholder="玩家账号" readonly></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='wanjia'" label="玩家姓名" prop="wanjiaxingming">
						<el-input v-model="sessionForm.wanjiaxingming" placeholder="玩家姓名" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='wanjia'" label="性别" prop="xingbie">
						<el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
							<el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='wanjia'" label="头像" prop="touxiang">
						<file-upload
							tip="点击上传头像"
							action="file/upload"
							:limit="1"
							...
							@change="wanjiatouxiangHandleAvatarSuccess"
							></file-upload>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='wanjia'" label="联系电话" prop="lianxidianhua">
...
<script>
	...
	export default {
		...
		created() {
			...
			if ('wanjia' == this.userTableName) {
				this.$set(this.sessionForm, 'wanjiazhanghao', null);
			}
			if ('wanjia' == this.userTableName) {
				this.$set(this.sessionForm, 'mima', null);
			}
			if ('wanjia' == this.userTableName) {
				this.$set(this.sessionForm, 'wanjiaxingming', null);
			}
			if ('wanjia' == this.userTableName) {
			...
			if ('wanjia' == this.userTableName&&this.rules['wanjiazhanghao']){
				this.rules['wanjiazhanghao'].push({ required: true, message: '请输入玩家账号', trigger: 'blur' })
			}else if('wanjia' == this.userTableName&&!this.rules['wanjiazhanghao']) {
				this.$set(this.rules, 'wanjiazhanghao', [{ required: true, message: '请输入玩家账号', trigger: 'blur' }]);
			}
			if ('wanjia' == this.userTableName&&this.rules['mima']){
				this.rules['mima'].push({ required: true, message: '请输入密码', trigger: 'blur' })
			}else if('wanjia' == this.userTableName&&!this.rules['mima']) {
				this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
			}
			if ('wanjia' == this.userTableName&&this.rules['wanjiaxingming']){
				this.rules['wanjiaxingming'].push({ required: true, message: '请输入玩家姓名', trigger: 'blur' })
			}else if('wanjia' == this.userTableName&&!this.rules['wanjiaxingming']) {
				this.$set(this.rules, 'wanjiaxingming', [{ required: true, message: '请输入玩家姓名', trigger: 'blur' }]);
			}
			if ('wanjia' == this.userTableName) {
		...
		methods: {
			...
			onSubmit(formName) {
				if(`wanjia` == this.userTableName && this.sessionForm.touxiang!=null){
			...
			wanjiatouxiangHandleAvatarSuccess(fileUrls) {
