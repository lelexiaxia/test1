<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="成绩" prop="chengji">
				<el-input-number v-model="ruleForm.chengji" placeholder="成绩" :disabled=" false ||ro.chengji"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item"  label="胜率" prop="shenglv">
				<el-select v-model="ruleForm.shenglv" placeholder="请选择胜率" :disabled=" false  ||ro.shenglv" >
					<el-option
			...
			<el-form-item class="add-item" label="MVP" prop="paiming">
				<el-input v-model.number="ruleForm.paiming" 
					placeholder="MVP" clearable :disabled=" false  ||ro.paiming"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`bisaichengji/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`bisaichengji/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
