...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/bisaileixing/bisaileixing').then(res => {
				if (res.data.code == 0) {
					this.bisaileixingOptions = res.data.data;
					this.selectOptionsList.push({name:'比赛类型',list:this.bisaileixingOptions,tableName: 'bisaileixing',check: -1})
				}
			});
