<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="比赛场次" prop="bisaichangci">
				<el-input v-model="ruleForm.bisaichangci" 
					placeholder="比赛场次" clearable :disabled=" false  ||ro.bisaichangci"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="对战选手" prop="duizhanxuanshou">
				<el-input v-model="ruleForm.duizhanxuanshou" 
					placeholder="对战选手" clearable :disabled=" false  ||ro.duizhanxuanshou"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="安排时间" prop="anpaishijian">
				<el-date-picker
					:disabled=" false  ||ro.anpaishijian"
			...
			<el-form-item class="add-item" label="对战安排" prop="duizhananpai">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`duizhananpai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`duizhananpai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
