	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/changdiyuyue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('changdiyuyue','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('changdiyuyue','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">预约备注</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.yuyuebeizhu"></div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/changdiyuyue', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此场地预约？') .then(_ => {
					this.$http.post('changdiyuyue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
